...
def columnise_nls_data(df: pd.DataFrame, file_path: Path, debug: bool) -> pd.DataFrame:
    """
    Reformat data provided by National Library of Scotland.

    Original data is 'dictionary' style format, with tab separated sets of key-value
    ...
    This function returns a dataframe in columnar format (one column per key) e.g.
    ...
    :param file_path: File path of original data
    :param debug: if True then save the dataframe to file
    ...
    if debug:
        ...
        df.to_csv(
            helpers.labelled_file(out_dir, file_path, "columnar", ".tsv"), sep="\t"
        )
    return df
...
def add_file_data_to_index(df: pd.DataFrame, file_path: Path):
    """
    Add a file identifier to the pandas dataframe index.

    Expects a file path like '/path/to/filename_<number>.txt'
    and adds <number> as a prefix to the index
    ...
    e.g. '/path/to/file_name.txt' outputs 'file_name:<row_number>'
    ...
    :return: Dataframe with updated index
    :rtype: pd.DataFrame
    """
    try:
...
def clean_nls_dates(df: pd.DataFrame, file_path: Path, debug: bool) -> pd.DataFrame:
    """
    ...
    question_dates: pd.Series = (
        dates_df.pop("question_date").groupby(level=0).first().dropna()
    )

    circa_dates: pd.Series = (
        dates_df.pop("circa_date").groupby(level=0).first().dropna()
    )

    # Grab the unqualified dates with original indices into one series,
    # if there are more than one, take the lowest
    min_uq_dates: pd.Series = (
        dates_df.groupby(level=0)
        .min()
        .rename(columns={"unqualified_date": "min_uq_date"})
        .dropna()
    )

    # Grab the unqualified dates with original indices into one series,
    # if there are more than one, take the highest
    max_uq_dates: pd.Series = (
        dates_df.groupby(level=0)
        .max()
        .rename(columns={"unqualified_date": "max_uq_date"})
        .dropna()
    )
    ...
    # the new frame has the same size/index as the original
    date_range = pd.DataFrame(np.nan, index=df.index, columns=["min_date", "max_date"])
    ...
    if debug:
        ...
        processed_dates.to_csv(
            helpers.labelled_file(out_dir, file_path, "processed_dates", ".tsv"),
            sep="\t",
        )
        df.to_csv(
            helpers.labelled_file(out_dir, file_path, "cleaned_dates", ".tsv"), sep="\t"
        )
...
def filter_nls_date(
    ...
    """
    Filter out entries that are within 'date_range' years from 'filter_date'.

    N.B. pandas' 'to_datetime()' can't deal with dates before 1678(!)
    so we're forcing all dates before then to 1678 since we're not interested
    in this time period

    :param df: The full dataframe of entries
    :type df: pd.DataFrame
    :param filter_date: date to filter, if 'None' then returns undated entries
    :type filter_date: int | None
    :param date_range: include dates +/- this value in years
    :type date_range: float
    ...
    if filter_date is not None:
        register_df = df.loc[
            ((df["min_date"] - mod_year) < filter_date)
            & ((df["max_date"] + mod_year) > filter_date)
        ]
        # Oh hey! A hack! Apparently datetime64[ns] format has a
        # problem with dates before 1678
        register_df.loc[:, "min_date"] = register_df.loc[:, "min_date"].map(
            lambda d: 1678.0 if d < 1678.0 else d
        )
    else:
